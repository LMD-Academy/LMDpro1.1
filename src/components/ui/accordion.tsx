...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b last:border-b-0", className)} // Removed last border for cleaner look in lists
    {...props}
  />
))
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      {children}
      <ChevronDown className="h-4 w-4 shrink-0 transition-transform duration-200 text-primary" /> {/* Added text-primary for icon color */}
    </AccordionPrimitive.Trigger>
